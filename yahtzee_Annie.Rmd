...
hand="FullHouse"}else if ((x[1]+1==x[2]&x[2]+1==x[3]&x[3]+1==x[4]&x[4]+1!=x[5])|(x[1]+1!=x[2]&x[2]+1==x[3]&x[3]+1==x[4]&x[4]+1==x[5])){
hand="smallstraight"
...
```{r}
# corrected function
a <- bubblesort(sample(1:6, size=5, replace=T))

hand <- function(k) {
  if(k[1]==k[5]) {
    hand <- "Yahtzee"
  } else if(k[1]==k[4] | k[2]==k[5]) {
    hand <- "4ofKind"
  } else if((k[1]==k[3] & k[4]==k[5]) | (k[3]==k[5] & k[1]==k[2]) ) {
    hand <- "FullHouse"
  } else if(k[2]==k[4] | k[1]==k[3] | k[3]==k[5]) {
    hand <- "3ofKind"
  } else if(k[2]==k[1]+1 & k[3]==k[2]+1 & k[4]==k[3]+1 & k[5]==k[4]+1) {
    hand <- "LargeStraight"
  } else if(sum(k[2]-k[1]==1,k[3]-k[2]==1,k[4]-k[3]==1,k[5]-k[4]==1)==3&k[3]-k[2]!=2&k[4]-k[3]!=2) {
    hand <- "SmallStraight"
  } else {
    hand <- "Chance"
  }
...
check_probability <- function(n) {
  ...
  df <- data.frame(Hand=outcome) %>%
    group_by(Hand) %>%
    summarise(Probability=n()/n)
  return(df)  
}

check_probability(100000)
```
...
```{r}
RepeatGame<-data.frame(Hand=replicate(100000,{x <- bubblesort(sample(1:6,size=5,replace=T))
Type(x)}))
install.packages("dplyr")
library(dplyr)
ProbTable<-RepeatGame%>%group_by(Hand)%>%summarise(Probability=n()/100000)
ProbTable
```
